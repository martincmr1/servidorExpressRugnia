...
class CartsManagerFs {
  constructor() {
    this.carritoFilePathFilePath = path.join(
      __dirname,
      "../db/fileSystem/carrito.json"
    );
  }
  ...
  async addProductToCart(req, res) {
    try {
      ...
      if (productId <= 0) {
        return res
          .status(400)
          .json({ error: "el ID debe ser mayor o igual a 1" });
      }
      const data = await fs.promises.readFile(this.carritoFilePath, "utf-8");
      const carts = JSON.parse(data);
      ...
      await fs.promises.writeFile(
        this.carritoFilePath,
        JSON.stringify(carts, null, 2),
        "utf-8"
      );
      res.status(200).json({
        message: `El producto con el ID ${pid} fue agregado correctamente al carrito ${cid}`,
      });
    } catch (error) {
      res.status(500).json({ error: "error en el servidor" });
    }
  ...
  async createCart(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ message: "Carrito agregado correctamente", newCart });
    } catch (error) {
      res.status(500).json({ error: "error en el servidor" });
    }
  }
